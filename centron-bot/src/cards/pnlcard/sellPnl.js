...
/**
 * Generate a PnL summary card image
 * @param {Object} data
 * @param {string} data.walletName - Name of the wallet
 * @param {string} data.walletAddress - Wallet address (optional)
 * @param {string} data.tokenSymbol - Token symbol (e.g., "JEETS")
 * @param {number} data.totalInvested - Total SUI spent
 * @param {number} data.totalReceived - Total SUI received
 * @param {number} data.profitLoss - Net profit/loss in SUI
 * @param {number} data.profitLossPercent - Profit/loss percentage
 * @param {number} data.amountSold - Amount of token sold
 * @param {string} data.txLink - Transaction link for QR code
...
export async function generatePnlCard(data) {
    ...
    // Draw PnL percentage (big box)
    // drawText(
    //     `${pnlPrefix}${data.profitLossPercent.toFixed(2)}%`,
    //     { ...templateConfig.positions.pnlPercent, color: pnlColor }
    // );
    ...
    const pnlDisplay = `$${Math.abs(data.profitLoss * 4.5).toFixed(0)}`; // Assuming SUI = $4.5
    // drawText(
    //     pnlDisplay,
    //     { ...templateConfig.positions.pnlAmount, color: pnlColor }
    // );
    ...
    // drawText("INVESTED", templateConfig.positions.investedLabel);
    drawText(
    ...
    // Draw SOLD section
    // drawText("SOLD", templateConfig.positions.soldLabel);
    drawText(
